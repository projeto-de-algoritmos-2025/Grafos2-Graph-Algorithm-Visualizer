...
export default function GraphCanvas({
  ...
  const [editingNode, setEditingNode] = useState<{ id: number; x: number; y: number } | null>(null)
  ...
  const handleMouseDown = (e: MouseEvent) => {
    ...
    const clickedNode = graphData.nodes.find(
      (node) => Math.sqrt(Math.pow(node.x - x, 2) + Math.pow(node.y - y, 2)) < NODE_RADIUS,
    )
    ...
    if (clickedNode) {
      if (activeTool === "move") {
        ...
        // Edit node ID
        setEditingNode({ id: clickedNode.id, x: clickedNode.x, y: clickedNode.y })
        setNewNodeId(clickedNode.id.toString())
      }
    } else if (activeTool === "node") {
      // Create a new node
      const nextId = graphData.nodes.length > 0 ? Math.max(...graphData.nodes.map((n) => n.id)) + 1 : 1
    ...
    if (!clickedNode) {
      for (const edge of graphData.edges) {
        ...
        if (sourceNode && targetNode) {
          if (isPointOnEdge(x, y, sourceNode.x, sourceNode.y, targetNode.x, targetNode.y)) {
        ...
        if (endNode && endNode.id !== edgeStartNodeId && onEdgeAdd) {
          ...
          const edgeExists = graphData.edges.some(
            (edge) =>
              ...
              (!isDirected && edge.source === endNode.id && edge.target === edgeStartNodeId),
          )
  ...
  }, [graphData, activeTool, draggedNodeId, edgeStartNodeId, onNodeAdd, onEdgeAdd, onNodeMove, onEdgeSelect])
  ...
  const handleNodeIdUpdate = () => {
    if (editingNode && onNodeUpdate) {
      ...
      if (!isNaN(newId) && newId > 0) {
        ...
        const idExists = graphData.nodes.some((node) => node.id === newId && node.id !== editingNode.id)
  ...
  const drawNode = (ctx: CanvasRenderingContext2D, node: Node, algorithmStep?: AlgorithmStep) => {
  ...
  const isPointOnEdge = (px: number, py: number, x1: number, y1: number, x2: number, y2: number, threshold = 10) => {
    const A = px - x1
    const B = py - y1
    const C = x2 - x1
    const D = y2 - y1
  ...
  return (
    <div className="relative">
      <canvas ref={canvasRef} width={width} height={height} className="border rounded-lg bg-white" />
