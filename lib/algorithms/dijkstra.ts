...
export function runDijkstra(
  ...
  while (unvisited.size > 0 && !foundTarget) {
    ...
    steps.push({
      ...
      stats: {
        "Nó Atual": current,
        Distância: distances[current] === Number.POSITIVE_INFINITY ? "∞" : distances[current],
        "Nós Visitados": visitedNodes.length,
...
const edges = graphData.edges.filter((e) => 
        ...
        if (distanceToNeighbor < distances[neighbor]) {
          ...
          steps.push({
            ...
            stats: {
              "Nó Atual": current,
              Vizinho: neighbor,
              "Distância Atual": distances[neighbor] === Number.POSITIVE_INFINITY ? "∞" : distances[neighbor],
  ...
  if (targetNode !== null) {
    ...
    pathNodes = nodes.filter(
      (node) => node === sourceNode || (distances[node] !== Number.POSITIVE_INFINITY && previous[node] !== null),
    )
  }

  // Calcular a distância total para o caminho
  let totalDistance = 0
  if (targetNode !== null && distances[targetNode] !== Number.POSITIVE_INFINITY) {
  ...
  const pathFound = targetNode === null || (targetNode !== null && distances[targetNode] !== Number.POSITIVE_INFINITY)
  ...
  const chosenEdgesFormatted = chosenEdges.map((edge) => `${edge.source} → ${edge.target}`).join(", ")
  ...
  steps.push({
    ...
    visitedNodes: nodes.filter((n) => distances[n] !== Number.POSITIVE_INFINITY),
